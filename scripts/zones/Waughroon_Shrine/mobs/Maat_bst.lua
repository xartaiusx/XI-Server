...
entity.onMobInitialize = function(mob)
    ...
    mob:addListener('TAKE_DAMAGE', 'MAAT_TAKE_DAMAGE', function(mobArg, damage, attacker, attackType, damageType)
        ...
        if damage >= 400 then
            ...
            mobArg:setLocalVar('miniEnrage', 1)
        end
    end)
end
...
entity.onMobFight = function(mob, target)
    ...
    then
        ...
        mob:useMobAbility(xi.mobSkill.FAMILIAR_MAAT)
        return
    end
...
entity.onMobWeaponSkill = function(mob, target, skill, action)
    ...
    if
        skillID == xi.mobSkill.FAMILIAR_MAAT and
        mob:getLocalVar('miniEnrage') == 1
    then
        if pet then
            pet:setMobMod(xi.mobMod.BASE_DAMAGE_MULTIPLIER, 200)
            mob:showText(mob, ID.text.NOW_THAT_IM_WARMED_UP)
            return
        end
    end

    if skillID == xi.mobSkill.FAMILIAR_MAAT then
        mob:showText(mob, ID.text.NOW_THAT_IM_WARMED_UP)
        return
    end
