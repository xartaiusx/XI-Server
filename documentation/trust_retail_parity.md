...
- `!trustparty repair` refreshes supported active Trust spell lists and applies runtime gambits without requiring a full party recast.
...
- Lua Trust scripts and module commands can hot-reload through the local file watcher, but already-summoned Trusts keep their existing gambit list. Use `!trustparty repair` for additive runtime fixes, and dismiss/resummon Qultada or Joachim to verify revised roll/song priority from a clean spawn.
...
- 2026-06-25 report snapshot: `C:\Users\xtyty\Documents\FFXI-Runtime\reports\trust-parity-audit-20260625-004811.md` shows 122 local Trust scripts, 24 explicit profiles, 98 generated audit profiles, 17 active Trusts in the latest combat log, no unresolved log names, `tank_assist` only on Amchuchu/August/Valaineral, and old pre-patch early-refresh rows to retest after the duration-aware C++ predicate rebuild. Runtime issue rows now include current battle target, gambit target, resolved action id, and select argument context so the next test can distinguish target churn from bad Trust selection.
