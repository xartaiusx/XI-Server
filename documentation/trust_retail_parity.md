...
- Before sending commands or taking screenshots, foreground the `Twills` xiloader/Windower window and run `C:\Github Repo's\FFXI\Runtime\client-tools\assert_windower_foreground.ps1`. Submit normal commands through `C:\Github Repo's\FFXI\Runtime\client-tools\Invoke-WindowerCommand.ps1`; pass `-AllowMutation` only for a deliberately reviewed mutating GM command, never for audits or status checks. Screenshots count as evidence only when `C:\Github Repo's\FFXI\Runtime\client-tools\capture_windower_window.ps1` triggers Windower's native `screenshot` command after `IsWindowerClient = true` and reports full client dimensions.
